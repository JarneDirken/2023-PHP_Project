        ...
        <x-slot name="content">
            <section class="border-b border-gray-300">
                ...
                <ul class="ml-2 mt-2 list-disc">
                    <li class="mb-2">Een overzicht bekijken van alle gemaakte orders van de leden, met bijhorende artikelen onder elkaar opgesomd.</li>
                    <li class="mb-2">Moest 1 van deze leden een kortingsbon ingegeven hebben deze ook op deze pagina waarnemen.</li>
                </ul>
            </section>
        </x-slot>
    </x-help-modal>
        ...
        @forelse($orders as $index=> $order)
            <div class="flex flex-col bg-white border border-gray-300 shadow-md rounded-lg custom-div overflow-hidden">
                <div class="p-2 bg-gray-100 border-b border-b-gray-300">
                    <p class="font-bold"><span class="font-normal">#{{$order->id}}</span> Bestelling van: {{ $order->date }}</p>
                    <p class="font-bold"> {{ $order->user->first_name . ' ' . $order->user->last_name }}
                    </p>
                </div>
                @php
